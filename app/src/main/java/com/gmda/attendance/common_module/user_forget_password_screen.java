...
public class user_forget_password_screen extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_user_forgot_password_screen);
        ...
        progressDialog = new ProgressDialog(user_forget_password_screen.this);
        ...
        btn_sentOtp.setOnClickListener(view -> {
            if (TextUtils.isEmpty(et_mobile.getText().toString())) {
                ...
                Toast.makeText(user_forget_password_screen.this, "Enter Correct No", Toast.LENGTH_SHORT).show();
            } else {
        ...
        btn_validate.setOnClickListener(v -> {
            if (et_otp.getText().toString().isEmpty()) {
                Toast.makeText(user_forget_password_screen.this, "Enter Otp ", Toast.LENGTH_SHORT).show();
            } else if (et_otp.getText().length() < 5) {
                Toast.makeText(user_forget_password_screen.this, "Enter Valid Otp", Toast.LENGTH_SHORT).show();
                        ...
                        if (response.isSuccessful()) {
                            try {
                                ...
                                if (api_result.equals("success")) {
                                    Toast.makeText(user_forget_password_screen.this, "Otp Verification Complete ", Toast.LENGTH_SHORT).show();
                                    startActivity(new Intent(user_forget_password_screen.this, user_forget_password_fill_screen.class));
                                    finish();
                                } else {
                                    progressDialog.dismiss();
                                    Toast.makeText(user_forget_password_screen.this, "" + api_message, Toast.LENGTH_SHORT).show();
                                }
                            } catch (Exception e) {
                                e.printStackTrace();
                                progressDialog.dismiss();
                ...
                if (response.isSuccessful()) {
                    try {
                        ...
                        if (api_result.equalsIgnoreCase("success")) {
                            JSONObject JsonResponseData = jsonObject.optJSONObject("responseData");
                            SharedPreferencesHandler s = new SharedPreferencesHandler(user_forget_password_screen.this);
                            s.DirectWritePreference("password_emp_id", JsonResponseData.optString("emp_id"), user_forget_password_screen.this);
                            send_otp_to_user();
                        } else {
                            progressDialog.dismiss();
                            Toast.makeText(user_forget_password_screen.this, "" + api_message, Toast.LENGTH_SHORT).show();
