...
public class UserDurationAttendanceAdapter extends RecyclerView.Adapter<UserDurationAttendanceAdapter.ViewHolder> {
   ...
   private List<UserDateWiseData.ResponseDataItem> list;

    public UserDurationAttendanceAdapter(Context context, List<UserDateWiseData.ResponseDataItem> list) {
    ...
    public void onBindViewHolder(@NonNull ViewHolder holder, int position) {
        ...
        final UserDateWiseData.ResponseDataItem readAttendance = list.get(position);
     ...
     /*  if (readAttendance.getPunchIn()!=null && readAttendance.getPunchOut()!=null){
        ...
        SharedPreferencesHandler sd = new SharedPreferencesHandler(context);
     ...
     //   holder.tvName.setText(sd.DirectReadPreference(Constdata.emp_name,context));
    ...
    // holder.tv_punchOutTime.setText(String.valueOf(readAttendance.getPunchOut()));
     ...
     holder.itemView.setOnClickListener(new View.OnClickListener() {
      @Override
          ...
          SharedPreferencesHandler sc = new SharedPreferencesHandler(context);
                Intent intent= new Intent(context, AttendanceViewIdActivity.class);
                intent.putExtra("emp_name",sc.DirectReadPreference(Constdata.emp_name,context));
                intent.putExtra("emp_id",readAttendance.getEmpId());
                intent.putExtra("designation",sc.DirectReadPreference(Constdata.designation,context));
                intent.putExtra("department",sc.DirectReadPreference(Constdata.department,context));
                intent.putExtra("division",sc.DirectReadPreference(Constdata.division,context));
             ...
             intent.putExtra("punch_out",readAttendance.getPunchOut());
              ...
              //  intent.putExtra("punchin_lat",readAttendance.getPunchinLat());
               // intent.putExtra("punchin_long",readAttendance.getPunchinLong());
                ...
                //intent.putExtra("data",readAttendance.getAStatus());
        /*  if (readAttendance.getPunchIn()!=null && readAttendance.getPunchOut()!=null){
              intent.putExtra("punch_in",outputFormat.format(date));
              intent.putExtra("punch_out",outputFormat1.format(date1));
             // holder.tv_punchInTime.setText(String.valueOf(outputFormat.format(date)));
              //holder.tv_punchOutTime.setText(String.valueOf(outputFormat1.format(date1)));
          }
          else if (readAttendance.getPunchIn()!=null && readAttendance.getPunchOut()==null){
